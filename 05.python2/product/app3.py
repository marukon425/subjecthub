...
class MoneyTxt(Money):
    ...
    def balance_init(self, balance_path) :
        ...
        try:
            ...
            # ファイルから残高を読み取り、残高を設定
            self.balance = int(self.balance_txt.get_txt())
...
class MoneyCsv(MoneyTxt):
    ...
    def __init__(self, balance_path,history_path):
        super().__init__(balance_path)
        self.history_path = history_path
        self.history_init(history_path)
    
    def history_init(self, history_path):
        try:
            self.balance_csv = FileCsv(history_path)
            self.balance = self.balance_csv.get_csv()
        except:
            self.histry = []

    # def save(self):
    #     self.balance_csv.set_csv(self.balance)
    def save(self):
        self.balance_txt.set_txt(self.balance)
        self.balance_csv.set_csv(self.histry)
...
print('--------家計簿アプリ--------')
# ファイル名を引数として
cm = MoneyCsv('balance.txt', 'history.csv')
while True:
    ...
    # 出金
        ...
        if not cm.set_withdrawal(in_subjects, in_pay):
            print('残高が足りないので出金できませんでした')
